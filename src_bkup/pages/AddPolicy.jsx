...
const AddPolicy = () => {
    ...
    useEffect(() => {
        ...
        if (policyNumber) {
            axios.get(`http://localhost:5000/api/policy/${policyNumber}`)
                .then((response) => {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/policy/${policyNumber}`, formData, {
    ...
    const handleAutoFill = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/api/autofill', autoFillData, {
