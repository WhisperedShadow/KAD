...
import styles from './PolicyTypes.module.css'; 
...
const PolicyTypes = () => {
    ...
    const handleCreate = async () => {
        try {
            if (editId) {
                await axios.put(`${backendUrl}/api/policytype/${editId}`, {
                    policyType,
                    comments,
                });
            } else {
                await axios.post(`${backendUrl}/api/policytype`, {
                    policyType,
                    comments,
                });
            }
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`${backendUrl}/api/policytype/${id}`);
            fetchpolicyTypes();
        } catch (error) {
    ...
    const handleEdit = (code) => {
        setpolicyType(code.policyType);
        setComments(code.comments);
        setEditId(code._id);
        setIsModalOpen(true);
    };

    const filteredpolicyTypes = policyTypes.filter(code =>
        code.policyType.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (
        <div className={styles.Container}>
          <div className={styles.ContentHeader}>
                <h2 className={styles.title}>Policy Types</h2>
            ...
            <table className={styles.CpyTable}>
                <thead>
                    <tr>
                        ...
                        <th className={styles.cpyName}>Policy Type</th>
                ...
                <tbody>
                    {filteredpolicyTypes.map((code) => (
                        <tr key={code._id}>
                            <td className={styles.cpyName}>{code.policyType}</td>
            ...
            {isModalOpen && (
                <div className={styles.modalOverlay}>
                    <div className={styles.modalContent}>
                        <h2 className={styles.modalTitle}>{editId ? 'Edit Policy Type' : 'Create Policy Type'}</h2>
